...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
    return page.evaluate((rootSelector) => {
        ...
        const styleOf = (el: Element, pseudo?: string): CSSStyleDeclaration => {
            const key = pseudo ?? '';
            const cached = (el as unknown as {
                __sc?: Map<string, CSSStyleDeclaration>;
            }).__sc?.get(key);
            if (cached)
                return cached;
        ...
        const backdropFrom = (from: Element | null, p: Point): RGBA => {
            ...
            while (node) {
                ...
                if (covers)
                    acc = over(acc, ownPaint(styleOf(node), r, p));
                if (acc.a >= 1)
                    break;
        ...
        for (const el of Array.from(document.querySelectorAll(CONTROL))) {
            ...
            if (fillOwn.a <= 0 && !hasBorder && !hasOutline)
                continue;
            const appearance = cs.appearance || (cs as unknown as {
                webkitAppearance?: string;
            }).webkitAppearance || 'none';
            const nativeWidget = ['checkbox', 'radio', 'range', 'color', 'file'].includes((el as HTMLInputElement).type ?? '');
            if (appearance !== 'none' && nativeWidget)
                continue;
            ...
            if (rounded < 3) {
                failures.push({
                    ...
                    selector: sel(el),
                    detail: `${how} — best delineator of a control that has ` +
            ...
            for (const pseudo of ['::before', '::after']) {
                ...
                if (rr < required) {
                    ...
                    failures.push({
                        ...
                        detail: `content ${content} at ${size}px/${weight}` +
